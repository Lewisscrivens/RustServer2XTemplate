...
{
    [Info("Steam Checks", "austinv900", "3.0.7")]
    ...
    public class SteamChecks : CovalencePlugin
    {
        ...
        private List<IPlayer> privateProfiles;
        private List<IPlayer> profileSharing;
        ...
        private bool IsPrivateProfile(IPlayer p) => privateProfiles.Contains(p);

        private bool IsPrivateProfile(Summaries s) => s.Response.Players[0].CommunityVisibilityState < 3;
        ...
        private bool IsSharing(IPlayer p) => profileSharing.Contains(p);
        ...
        {
            return g.Response.GameCount != null ? g.Response.GameCount : 0;
        }
        ...
        private void Init()
        {
            privateProfiles = new List<IPlayer>();
            profileSharing = new List<IPlayer>();
        ...
        {
            if (profileSharing.Contains(player))
            {
                profileSharing.Remove(player);
            }

            if (privateProfiles.Contains(player))
            {
                privateProfiles.Remove(player);
            }
        }
        ...
        private void CheckSummaries(IPlayer player)
        {
            webrequest.Enqueue(string.Format(RootUrl + PlayerStr, apiKey, player.Id), null, (code, response) =>
            {
                ...
                if (IsPrivateProfile(summaries))
                {
                    LogWarning("Private profile!");
                    if (!IsPrivateProfile(player))
                    {
                        privateProfiles.Add(player);
                    }
                }

                if (IsPrivateProfile(player) && kickPrivateProfile)
                {
        ...
        private void CheckSharing(IPlayer player)
        {
            webrequest.Enqueue(string.Format(RootUrl + SharingStr, apiKey, player.Id, appId), null, (code, response) =>
            {
                ...
                if (IsSharing(sharing))
                {
                    if (!IsSharing(player))
                    {
                        profileSharing.Add(player);
                    }
                }

                if (IsSharing(player) && kickFamilyShare)
                {
