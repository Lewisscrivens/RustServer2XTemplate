...
{
    [Info("Phantom Sleepers", "nivex", "0.1.4")]
    ...
    class PhantomSleepers : RustPlugin
    {
        ...
        [ConsoleCommand("createphantom")]
        void ccmdCreatePhantom(ConsoleSystem.Arg arg)
        {
            if (!init || arg.Player() == null)
                return;

            var player = arg.Player();
            if (!arg.IsAdmin && !permission.UserHasPermission(player.userID.ToString(), permissionName))
            {
                player.ChatMessage(string.Format("Permission denied. Requires access to {0}", permissionName));
                return;
            }
        ...
        {
            if (init && target?.GetComponent<Phantom>() != null && player && !player.IsAdmin)
        ...
        object CanNetworkTo(BasePlayer player, BasePlayer target)
        {
            ...
            if (sleepers.Contains(player.net.ID))
                return false;
